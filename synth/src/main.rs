    ...
    T: SizedSample + FromSample<f64>,
{
    ...
    let pan_amount = shared(0.0);
    let left_volume = var_fn(&pan_amount, |x| 1.0 - clamp(0.0, 1.0, x)) >> follow(0.01);
    let right_volume = var_fn(&pan_amount, |x| 1.0 - clamp(0.0, 1.0, -x)) >> follow(0.01);
    ...
    net = net >> (left_volume * pass() | right_volume * pass());
    net = net
...
impl eframe::App for State {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.add(egui::Slider::new(&mut pan, -1.0..=1.0).suffix("%"));
